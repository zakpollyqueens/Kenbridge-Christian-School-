...
try{
const{rows}=await pool.query(
"SELECT bd.*, u.full_name AS uploader_name, m.title AS meeting_title, m.meeting_date FROM board_documents bd LEFT JOIN users u ON u.id=bd.uploaded_by LEFT JOIN board_meetings m ON m.id=bd.meeting_id ORDER BY bd.created_at DESC"
);
return res.status(200).json({
success:true,
documents:rows
...
const{rows}=await pool.query(
"INSERT INTO board_documents( title,description,document_type,file_url,storage_path, meeting_id,uploaded_by,is_confidential,created_at ) VALUES($1,$2,$3,$4,$5,$6,$7,$8,NOW()) RETURNING *",
[
String(title).trim(),
description?String(description).trim():null,
type,
...
const{rows}=await pool.query(
"UPDATE board_documents SET title=$1, description=$2, document_type=$3, file_url=$4, storage_path=$5, meeting_id=$6, is_confidential=$7 WHERE id=$8 RETURNING *",
[
String(title).trim(),
description?String(description).trim():null,
type,
...
try{
const{rows}=await pool.query(
"DELETE FROM board_documents WHERE id=$1 RETURNING id,title",
[req.params.id]
);
...
try{
const{rows}=await pool.query(
"SELECT br.*, u.full_name AS proposer_name, m.title AS meeting_title, m.meeting_date FROM board_resolutions br LEFT JOIN users u ON u.id=br.proposed_by LEFT JOIN board_meetings m ON m.id=br.meeting_id ORDER BY br.created_at DESC"
);
...
router.post("/resolutions",async(req,res)=>{
 ...
 try{
  ...
  const proposer=proposed_by||admin.id;
  ...
  if(error.code==="23503"){
   return res.status(400).json({
    success:false,
    message:"The selected meeting or proposer could not be found."
   });
  }
...
/* UPDATE RESOLUTION STATUS - ADMIN */
router.patch("/resolutions/:id/status",async(req,res)=>{
 const admin=await requireAdmin(req,res);
 if(!admin)return;
 ...
 try{
    ...
    SET status=$1,
        decision_date=
          CASE
           ...
           THEN COALESCE(decision_date,NOW())
           ELSE decision_date
          END
    WHERE id=$2
    RETURNING *`,
  ...
  console.error(
   "UPDATE RESOLUTION STATUS ERROR:",
   error.message
  );

  return res.status(500).json({
   success:false,
   message:"Unable to update resolution status."
  });
 }
});

/* DELETE BOARD RESOLUTION - ADMIN */
 ...
 try{
  const{rows}=await pool.query(
   `SELECT
    ...
    ORDER BY
      COALESCE(ba.published_at,ba.created_at) DESC`
  );
...
/* CREATE BOARD ANNOUNCEMENT - ADMIN */
 ...
 try{
  ...
  const{rows}=await pool.query(
   ...
   [
    String(title).trim(),
    String(message).trim(),
    priority?String(priority).toUpperCase():"NORMAL",
    is_active!==false,
    published_at||null,
    admin.id
   ]
  ...
  console.error(
   "CREATE BOARD ANNOUNCEMENT ERROR:",
   error.message
  );
...
/* UPDATE BOARD ANNOUNCEMENT - ADMIN */
router.put("/announcements/:id",async(req,res)=>{
 ...
 try{
  ...
  console.error(
   "UPDATE BOARD ANNOUNCEMENT ERROR:",
   error.message
  );
...
/* DELETE BOARD ANNOUNCEMENT - ADMIN */
router.delete("/announcements/:id",async(req,res)=>{
 ...
 try{
  ...
  console.error(
   "DELETE BOARD ANNOUNCEMENT ERROR:",
   error.message
  );
...
const{rows}=await pool.query(
"UPDATE board_meetings SET title=$1, description=$2, meeting_date=$3, start_time=$4, end_time=$5, meeting_type=$6, location=$7, online_provider=$8, meeting_url=$9, meeting_id=$10, passcode=$11, status=$12, updated_at=NOW() WHERE id=$13 RETURNING *",
[
String(title).trim(),
description?String(description).trim():null,
meeting_date,
...
try{
const{rows}=await pool.query(
"DELETE FROM board_meetings WHERE id=$1 RETURNING id",
[req.params.id]
);
...
try{
const{rows}=await pool.query(
"SELECT * FROM board_agenda_items WHERE meeting_id=$1 ORDER BY item_number ASC",
[req.params.id]
);
...
const{rows}=await pool.query(
"INSERT INTO board_agenda_items( meeting_id, item_number, title, description, presenter, status, created_at, updated_at ) VALUES($1,$2,$3,$4,$5,$6,NOW(),NOW()) RETURNING *",
[
req.params.id,
Number(item_number),
String(title).trim(),
...
const{rows}=await pool.query(
"UPDATE board_agenda_items SET item_number=$1, title=$2, description=$3, presenter=$4, status=$5, updated_at=NOW() WHERE id=$6 RETURNING *",
[
Number(item_number),
String(title).trim(),
description?String(description).trim():null,
...
try{
const{rows}=await pool.query(
"DELETE FROM board_agenda_items WHERE id=$1 RETURNING id",
[req.params.id]
);
...
router.post("/documents",async(req,res)=>{
 ...
 try{
  ...
  if(!file_url&&!storage_path){
   return res.status(400).json({
    success:false,
    message:"A document file or file path is required."
   });
  }
  ...
  const{rows}=await pool.query(
   ...
   [
    String(title).trim(),
    description?String(description).trim():null,
    document_type?String(document_type).trim():"document",
    file_url?String(file_url).trim():null,
    storage_path?String(storage_path).trim():null,
    meeting_id||null,
    admin.id,
  ...
  return res.status(201).json({
   success:true,
   message:"Board document uploaded successfully.",
   document:rows[0]
  });
 }catch(error){
  console.error(
   "CREATE BOARD DOCUMENT ERROR:",
   error.message
  );
 ...
 try{
  const{rows}=await pool.query(
   `SELECT
    ...
    LEFT JOIN board_meetings m ON m.id=r.meeting_id
...
router.get("/resolutions/:id",async(req,res)=>{
 ...
 try{
  ...
  console.error(
   "GET BOARD RESOLUTION ERROR:",
   error.message
  );

  return res.status(500).json({
   success:false,
   message:"Unable to retrieve Board resolution."
  });
 }
});
