...
VALUES($1,$2,$3,$4,$5,$6)`,
[user.id,String(full_name).trim(),cleanEmail,"ADMIN",null,true]
);
}catch(databaseError){
...
console.error("LOAD STAFF ERROR:",error.message);
...
if(req.params.id===adminAuth.authUser.id){
return res.status(400).json({
success:false,
...
if(authDeleteError){
console.error("SUPABASE DELETE STAFF ERROR:",authDeleteError.message);
...
console.error("DATABASE DELETE STAFF ERROR:",databaseError.message);
