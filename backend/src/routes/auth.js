...
if (!full_name || !username || !position || !department || !email || !password)
  return res.status(400).json({ success: false, message: "Required staff fields are missing." });
...
try {
  ...
  return res.status(500).json({ success: false, message: "Staff profile could not be saved." });
}
...
if (!profile)
  return res.status(403).json({ success: false, message: "Account is not registered in the staff system." });
