...
throw new Error("Your account does not have permission to access student records.");
}
...
FROM students
WHERE
($1::text IS NULL OR UPPER(TRIM(class_name))=$1)
AND
(
$2::text=''
...
OR LOWER(COALESCE(parent_name,'')) LIKE LOWER('%'||$2||'%')
)
ORDER BY
...
:normaliseStatus(existing.status);
