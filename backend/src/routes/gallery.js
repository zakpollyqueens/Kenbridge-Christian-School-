...
return res.status(500).json({success:false,message:"Unable to load gallery images."});
}
});
...
const {rows}=await pool.query(`SELECT g.id,g.title,g.description,g.image_url,g.storage_path,g.category,g.is_published,g.created_at,g.updated_at,u.full_name AS uploaded_by_name FROM gallery_images g LEFT JOIN users u ON u.id=g.uploaded_by ORDER BY g.created_at DESC`);
return res.status(200).json({success:true,images:rows});
}catch(error){
console.error("ADMIN GALLERY ERROR:",error.message);
return res.status(403).json({success:false,message:error.message||"Unable to load gallery management data."});
}
});
...
if(!imageUrl){
await supabase.storage.from("school-gallery").remove([storagePath]);
return res.status(500).json({success:false,message:"Image URL could not be created."});
}
...
[title,description,imageUrl,storagePath,category,admin.id,isPublished]
);
...
console.error("GALLERY DATABASE ERROR:",databaseError.message);
await supabase.storage.from("school-gallery").remove([storagePath]);
return res.status(500).json({success:false,message:"Image was uploaded but its gallery record could not be saved."});
}

}catch(error){
console.error("GALLERY UPLOAD ERROR:",error.message);
return res.status(403).json({success:false,message:error.message||"Unable to upload gallery image."});
}
});
...
if(!title||!image_url)return res.status(400).json({success:false,message:"Image title and image URL are required."});
...
return res.status(201).json({success:true,message:"Gallery image added successfully.",image:rows[0]});
}catch(error){
console.error("CREATE GALLERY IMAGE ERROR:",error.message);
return res.status(403).json({success:false,message:error.message||"Unable to add gallery image."});
}
});
...
if(!rows.length)return res.status(404).json({success:false,message:"Gallery image was not found."});

return res.status(200).json({success:true,message:"Gallery image updated successfully.",image:rows[0]});
}catch(error){
console.error("UPDATE GALLERY IMAGE ERROR:",error.message);
return res.status(403).json({success:false,message:error.message||"Unable to update gallery image."});
}
});
...
const {rows}=await pool.query(
`DELETE FROM gallery_images WHERE id=$1 RETURNING id,title,storage_path`,
[req.params.id]
);
...
return res.status(403).json({success:false,message:error.message||"Unable to delete gallery image."});
}
});
